...
if(isset($_GET['listar'])){
    $sql = "SELECT id, usuario, tipo, descripcion, cantidad, precio, fecha
            FROM gastos
...
if($_SERVER['REQUEST_METHOD']==='POST'){
    ...
    $sql = "INSERT INTO gastos (negocio, usuario, tipo, descripcion, cantidad, precio, fecha)
            VALUES ('".$_SESSION['nombre_bd_negocio']."','$usuario','$tipo','$descripcion',$cantidad,$precio,'$fecha')";
    if(mysqli_query($conexion,$sql)){
        echo json_encode(["ok"=>true,"msg"=>"✅ Gasto registrado"]);
    } else {
