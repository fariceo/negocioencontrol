<?php
/*
  archivo: sistema_opciones_binarias_compuesto.php
  Sistema en un solo archivo con:
  - Interés compuesto (%1 o %10)
...
$createSql = "CREATE TABLE IF NOT EXISTS operaciones (
...
if ($action) {
    ...
    try {
        if ($action === 'create') {
            ...
            $stmt = $pdo->prepare("INSERT INTO operaciones (fecha,hora_entrada,hora_expiracion,activo,direccion,monto,retorno_porc,resultado,ganancia_perdida,comentario) VALUES (:fecha,:he,:hx,:activo,:direccion,:monto,:ret,:resultado,:ganancia,:comentario)");
            $stmt->execute([
                ':fecha'=>$data['fecha'],
        ...
        if ($action === 'list') {
            $stmt = $pdo->query('SELECT * FROM operaciones ORDER BY creado_en DESC');
        ...
        if ($action === 'delete') {
            $id = (int)($_GET['id'] ?? 0);
            $stmt = $pdo->prepare('DELETE FROM operaciones WHERE id=:id');
            $stmt->execute([':id'=>$id]);
        ...
        if ($action==='stats') {
            $res = $pdo->query('SELECT SUM(CASE WHEN resultado="Win" THEN 1 ELSE 0 END) AS wins,
                ...
                FROM operaciones')->fetch();
            $wins = (int)$res['wins'];
            $losses = (int)$res['losses'];
            $total = $wins+$losses;
            $winrate = $total?round($wins/$total*100,2):0;
            $capital = $CAPITAL_INICIAL + (float)$res['net'];
            echo json_encode(['ok'=>true,'wins'=>$wins,'losses'=>$losses,'total'=>$total,'winrate'=>$winrate,'net'=>(float)$res['net'],'capital'=>$capital]);
            exit;
        }
...
<title>Opciones Binarias - Interés Compuesto y Recuperación</title>
<style>
...
<h1>Opciones Binarias con Interés Compuesto y Recuperación</h1>
...
let capitalActual = 0;
let perdidasConsecArray = []; // array para pérdidas consecutivas
...
async function loadStats(){
  ...
  document.getElementById('statsContent').innerHTML=`Total: ${r.total} operaciones<br>Wins: ${r.wins} | Losses: ${r.losses} | Winrate: ${r.winrate}%<br>Resultado neto: ${r.net}<br>Capital actual: ${capitalActual}`;

  // monto inicial según interés compuesto
  let montoIni = capitalActual<100?capitalActual*0.10:capitalActual*0.01;
  ...
  if(perdidasConsecArray.length > 0){
      ...
      const beneficioDeseado = 1; // siempre ganar al menos $1
      montoRec = (totalPerdidas + beneficioDeseado) / (retorno/100);

      // limitar al 50% del capital actual
  ...
  // asignar al input el monto sugerido
...
document.getElementById('tradeForm').addEventListener('submit',async(e)=>{
  ...
  if(r.ok){
    //alert('Guardado! Ganancia: '+r.ganancia);
    f.reset();
    if(r.ganancia<0){
